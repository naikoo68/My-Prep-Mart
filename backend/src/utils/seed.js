// Seed script: populates the DB with admin + student users, all subjects,
// sessions, questions and several test series. Run with: npm run seed
...
// Curated questions for Physics > Units & Measurements; everything else uses
// generated practice questions so the whole catalog is playable.
const CURATED = {
  "Units & Measurements": [
...
async function seed() {
  ...
  // A few extra users so the leaderboard / admin user list look real
  ...
  for (const s of SUBJECTS) {
    ...
    for (let i = 0; i < titles.length; i++) {
      ...
      const qs = buildQuestions(titles[i]).map((q) => ({
        ...q,
        subject: subject._id,
        session: session._id,
      }));
  ...
  // Test series (pull questions from the seeded bank)
  ...
  // Enroll the demo student in a few test series
  student.enrolledTests = [tests[0]._id, tests[1]._id, tests[4]._id];
  await student.save();

  // A few sample attempts for the demo student so the dashboard has data
  ...
  console.log("  Admin:   admin@myprepmart.com / admin123");
  console.log("  Student: student@myprepmart.com / student123");
  process.exit(0);
}
